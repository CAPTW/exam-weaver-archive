...
QUESTION_START = re.compile(
    r"(?<![\dA-Za-z])([1-9]\d?)\s*"
    r"(?:[\.•ㆍ·]|(?=\s*(?:다음|디음|자이로|DGPS|IAMSAR|해상\s+레이더|「|<|〈|보기)))"
    r"\s*",
    re.IGNORECASE,
)
...
KNOWN_GROUPS: dict[str, list[KnownGroup]] = {
    ...
    "[기출문제]경찰직 항해학(24년-13년).pdf": [
        ...
        KnownGroup(2013, "", "제2차", "경정", 20),
        KnownGroup(2013, "", "제2차", "순경", 20),
        KnownGroup(2013, "", "제1회", "순경", 20),
    ],
}

KNOWN_PAGE_RANGES: dict[str, list[tuple[int, int]]] = {
    "[기출문제]경찰직 항해학(24년 하반기-25년 하반기).pdf": [
        (1, 4), (5, 11), (12, 15), (16, 18), (19, 24),
    ],
    "[기출문제]경찰직 항해학(24년-13년).pdf": [
        (2, 4), (5, 7), (8, 14), (15, 17), (18, 21), (22, 25),
        (26, 32), (33, 36), (37, 39), (40, 42), (43, 45), (46, 48),
        (49, 51), (52, 54), (55, 57), (58, 61), (62, 64), (65, 66),
        (67, 68), (69, 71), (72, 74), (75, 76), (77, 78), (79, 80),
    ],
}
...
def looks_like_navigation_question_context(context: str) -> bool:
    if re.match(r"(?:항\s*해\s*(?:학|술)|매\s*학\s*술)\b", context):
        return False
    cues = [
        r"다음",
        r"보기",
        ...
        r"레이더",
        r"컴퍼스",
        r"GPS|GNSS|GMDSS|AIS|ECDIS|IALA",
        r"선박",
        r"수심",
        r"항로표지",
        r"기상|태풍|전선|일기도",
...
def question_start_candidates(text: str, max_question_number: int = 40) -> list[tuple[int, int, int]]:
    ...
    for match in QUESTION_START.finditer(text or ""):
        ...
        if len(context) < 8:
            continue
        if not looks_like_navigation_question_context(context):
            continue
...
def classify_topic(raw_text: str, placeholder: bool = False) -> TopicResult:
    ...
    rules: list[tuple[list[str], TopicResult]] = [
        ([r"속력", r"노트", r"해리", r"마일", r"시간", r"소요", r"거리", r"조시", r"조고", r"조고비", r"평균해면", r"자차", r"편차", r"나침로", r"진침로"],
         TopicResult("계산항해", "속력·거리·방위 계산", "항정·조석·자차편차 계산", ["계산", "속력", "방위"], 0.88)),
        ([r"교차방위", r"격시관측", r"동시관측", r"선위", r"위치선", r"중시선", r"수평협각", r"육분의", r"추측위치", r"추정위치", r"항정선", r"대권"],
         TopicResult("항법·선위측정", "연안항법", "방위관측·위치선·추측항법", ["선위", "연안항법"], 0.92)),
        ([r"해도", r"점장도", r"총도", r"항양도", r"해안도", r"항박도", r"수로서지", r"소개정", r"해도도식", r"등심선", r"저질"],
         TopicResult("해도·수로서지", "해도도식·수로정보", "해도 분류·기호·소개정", ["해도", "수로서지"], 0.92)),
        ([r"자이로", r"컴퍼스", r"레이더", r"ARPA", r"AIS", r"GPS", r"GNSS", r"GMDSS", r"ECDIS", r"전자해도", r"선속계", r"도플러", r"Echo sounder", r"음향측심", r"VDR"],
         TopicResult("항해계기·전자항법", "항해장비", "컴퍼스·레이더·GNSS·ECDIS", ["항해계기", "전자항법"], 0.92)),
        ([r"조석", r"조류", r"해류", r"유향", r"유속", r"정조", r"고조", r"저조", r"조차", r"부진동", r"해면"],
         TopicResult("조석·해류", "해양 현상", "조석·조류·해류 용어", ["조석", "조류"], 0.90)),
        ([r"일기도", r"기압", r"전선", r"태풍", r"풍향", r"풍속", r"운량", r"파랑", r"황천", r"저기압", r"고기압", r"cyclone|Typhoon"],
         TopicResult("해상기상", "기상·태풍", "일기도·전선·태풍 피항", ["기상", "태풍"], 0.90)),
        ([r"조선", r"묘박", r"투묘", r"양묘", r"주묘", r"예인", r"계류", r"선회", r"타판", r"타\(", r"Rudder", r"스러스터", r"인명구조", r"Williamson", r"Scharnov", r"Single turn"],
         TopicResult("조선·선박운용", "조종·묘박·계류", "조선법·예인·인명구조", ["조선", "선박운용"], 0.90)),
        ([r"복원성", r"GM", r"KG", r"KM", r"만재.?흘수", r"만재.?홀수", r"트림", r"흘수", r"홀수", r"부력", r"선박의 길이", r"이중저", r"용골", r"선저", r"톤수", r"선박검사"],
         TopicResult("선박구조·복원성", "선체·복원성", "흘수·GM·선박 구조", ["복원성", "선박구조"], 0.90)),
        ([r"IALA", r"부표", r"항로표지", r"등대", r"등질", r"등색", r"광달거리", r"두표", r"안전수역표지", r"방위표지", r"국제기류", r"기류신호", r"조종신호", r"KOSREP"],
         TopicResult("항로표지·신호", "부표·등화·신호", "IALA·등대표·해상신호", ["항로표지", "신호"], 0.92)),
        ([r"해사안전법", r"국제해상충돌예방", r"COLREG", r"동력선", r"범선", r"피항", r"등화", r"형상물", r"조종능력", r"홀수제약선"],
         TopicResult("항법규정·충돌예방", "해상충돌예방규칙", "피항관계·등화·형상물", ["충돌예방", "항법규정"], 0.88)),
    ]
...
def estimate_difficulty(text: str, topic: TopicResult, placeholder: bool = False) -> tuple[str, float]:
    if placeholder:
        return "unknown", 0.5
    if contains_any(text, [r"모두 몇", r"합", r"숫자", r"순서대로", r"계산", r"속력", r"거리", r"해리", r"노트", r"자차", r"편차", r"조시", r"조고"]):
        return "high", 1.0
    if topic.l1 in {"계산항해", "항법·선위측정", "항해계기·전자항법"}:
        return "medium", 0.6
    return "medium", 0.6
...
def cognitive_skill(text: str, placeholder: bool = False) -> str:
    if placeholder:
        return "unknown"
    if contains_any(text, [r"선박이", r"항해 중", r"항해중", r"상황", r"조치", r"피항", r"구조"]):
        return "case_application"
    if contains_any(text, [r"속력", r"거리", r"해리", r"노트", r"몇 m", r"몇 분", r"자차", r"편차", r"조시", r"조고", r"총합"]):
        return "calculation"
...
- 항해학은 20문항 채용형과 40문항 경위/간부후보형이 섞여 있어 세트별 문항 수를 별도로 관리했다.
...
## 주제 Taxonomy
- 계산항해, 항법·선위측정, 해도·수로서지, 항해계기·전자항법, 조석·해류, 해상기상, 조선·선박운용, 선박구조·복원성, 항로표지·신호, 항법규정·충돌예방으로 분류했다.
...
def write_strategy(output_dir: Path, priority_rows: list[dict], question_rows: list[dict]) -> None:
    ...
    text = f"""# 항해학 기출 분석 기반 공부 전략

## 한눈에 보는 결론
- 분석 문항: {total_questions}개
- 자동 분류 후 검토 필요: {review_count}개
- 최근 3개년({", ".join(map(str, recent_years))})은 계산항해, 항법·선위측정, 항해계기·전자항법, 조석·기상, 항로표지 비중이 높다.
- 40문항 경위/간부후보형은 계산형과 장비·신호·조선 지식을 섞어 묻는 문항이 많아 공식과 개념을 같이 정리해야 한다.
...
## 추천 공부 순서
1. 계산항해: 속력·거리·시간, 자차·편차, 조석표 계산을 반복한다.
2. 항법·선위측정: 교차방위법, 위치선, 추측위치·추정위치, 해도 작도 절차를 정리한다.
3. 항해계기·전자항법: 컴퍼스, 레이더, GNSS, AIS, ECDIS, GMDSS의 원리와 오차를 묶는다.
4. 조석·해상기상: 조석·조류 용어와 일기도·전선·태풍 피항을 함께 학습한다.
5. 항로표지·신호와 조선: IALA, 등질·두표, 조종신호, 묘박·계류·예인·구조법을 오답 중심으로 정리한다.

## 4주 루틴
- 1주차: 계산항해 공식과 해도·선위측정 절차를 문항별로 재풀이한다.
- 2주차: 항해계기, 전자항법, 항로표지·신호를 장비/기호/오차 기준으로 정리한다.
- 3주차: 조석·조류, 해상기상, 조선·선박운용을 상황형 문항으로 복습한다.
- 4주차: 최근 3개년 20문항/40문항 세트를 시간 제한으로 풀고 오답을 주제별로 묶는다.

## 오답노트 템플릿
| 날짜 | 연도/구분 | 문제번호 | 주제 | 틀린 이유 | 정답 근거 | 재풀이일 |
|---|---|---:|---|---|---|---|
|  |  |  |  | 공식 적용 / 개념 혼동 / 부정문 실수 / OCR 확인 |  |  |
"""
...
def main() -> int:
    ...
    if len(groups) != expected:
        ...
        raise RuntimeError(f"Expected {expected} navigation groups, got {len(groups)}: {dict(by_file)}")
